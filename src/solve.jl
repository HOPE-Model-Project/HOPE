...
function has_feasible_primal_solution(model::Model)
    ...
    return result_count(model) > 0 &&
           pr_status in (MOI.FEASIBLE_POINT, MOI.NEARLY_FEASIBLE_POINT)
end

function require_feasible_primal_solution(
    model::Model;
    context::AbstractString = "Model solve",
)
    ...
    if !has_feasible_primal_solution(model)
        throw(
            ArgumentError(
                "$(context) did not find a feasible primal solution. termination_status=$(term_status), primal_status=$(pr_status), result_count=$(n_results)",
            ),
        )
    end
    return nothing
...
function solve_model(config_set::Dict, input_data::Dict, model::Model)
    ...
    write_shadow_prices =
        write_shadow_prices_raw isa Integer ? Int(write_shadow_prices_raw) :
        parse(Int, string(write_shadow_prices_raw))
    ...
    if write_shadow_prices == 1
        ...
        elseif model_mode == "GTEP"
            ...
            inv_dcs_bin_mode =
                inv_dcs_bin_raw isa Integer ? Int(inv_dcs_bin_raw) :
                parse(Int, string(inv_dcs_bin_raw))
    ...
    if need_lp_reresolve
        if solver_name == "cbc"
            println(
                "write_shadow_prices=1 requested, but CBC does not provide dual prices for this workflow. Skipping LP re-solve.",
            )
        else
            if has_primal_solution
                println(
                    "write_shadow_prices=1: fixing discrete variables and re-solving LP for dual prices...",
                )
                ...
                try
                    ...
                    if has_duals(model)
                        println(
                            "LP re-solve complete. Dual prices are available for output.",
                        )
                    else
                        println(
                            "LP re-solve complete, but dual prices are still unavailable from solver/model state.",
                        )
                    end
                catch e
                    println(
                        "Warning: LP re-solve for shadow prices failed. Keeping MILP solution. Error: $(e)",
                    )
                ...
                println(
                    "Skipping LP re-solve for shadow prices because no feasible MILP point is available (primal_status=$(pr_status)).",
                )
            end
        end
    end
