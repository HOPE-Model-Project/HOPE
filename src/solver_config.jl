function initiate_solver(case::AbstractString, solver::AbstractString)
    ...
    if solver == "scip"
        if !isdefined(@__MODULE__, :SCIP)
            try
                @eval using SCIP
            catch e
                error(
                    "solver='scip' requested, but SCIP.jl is unavailable in this environment. Install/activate SCIP.jl. Original error: $(e)",
                )
            end
        end
        # Optional solver parameters ############################################
    ...
    if solver == "cplex"
        if !isdefined(@__MODULE__, :CPLEX)
            try
                ...
                error(
                    "solver='cplex' requested, but CPLEX.jl is unavailable in this environment. Install/activate CPLEX.jl and a valid CPLEX license. Original error: $(e)",
                )
            end
        end
        # Optional solver parameters ############################################
    ...
    if solver == "gurobi"
        if !isdefined(@__MODULE__, :Gurobi)
            try
                ...
                error(
                    "solver='gurobi' requested, but Gurobi.jl is unavailable in this environment. Install/activate Gurobi.jl and a valid Gurobi license. Original error: $(e)",
                )
            end
        end
        # Optional solver parameters ############################################
