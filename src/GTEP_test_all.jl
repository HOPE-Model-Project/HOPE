...
#	@variable(model, ni[T,H,I])
	...
	for w in W
		...
		println(
			...
			">=",
			sum(sum(value(pwe[g,w,w_prime]) for w_prime in WER_w[w]) for g in intersect(union([G_i[i] for i in I_w[w]]...),union(G_PV,G_W));init=0)
	...
	for w in ["MD"]
		...
		for g in intersect(union([G_i[i] for i in I_w[w]]...),union(G_PV,G_W))
			println(
				value(pw[g,w]),
				">=",
				sum(value(pwe[g,w,w_prime]) for w_prime in WER_w[w]) 
	...
	for w in W
		for w_prime in WIR_w[w]
			...
			println(
				...
				">=",
				sum(value(pwe[g,w_prime,w]) for g in intersect(union([G_i[i] for i in I_w[w_prime]]...),union(G_PV,G_W));init=0)
	...
	for w in W
		...
		println(
			sum(value(pwe[g,w_prime,w])  for w_prime in WIR_w[w] for g in intersect(union([G_i[i] for i in I_w[w_prime]]...),union(G_PV,G_W)); init=0),
			"-", sum(value(pwe[g,w,w_prime]) for w_prime in WER_w[w] for g in intersect(union([G_i[i] for i in I_w[w]]...),union(G_PV,G_W)); init=0),
			"+",value(pt_rps[w]),
