...
function unit_commitment!(config_set::Dict, input_data::Dict, model::Model)
	...
	for g in G_UC
		ut_start = round(Int, UT_g[g]) + 1
		MUT_con_g = @constraint(model, [h in ut_start:H[end]], sum(su[g,hr] for hr in max(1, h-round(Int, UT_g[g])+1):h) <= o[g,h], base_name = "MUT_con_$g")
	end
		
	#(27) Minimum down time constraint
	for g in G_UC
		dt_start = round(Int, DT_g[g]) + 1
		MDT_con_g = @constraint(model, [h in dt_start:H[end]], sum(su[g,hr] for hr in max(1, h-round(Int, DT_g[g])+1):h) <= 1-o[g,h], base_name = "MDT_con_$g")
	end
