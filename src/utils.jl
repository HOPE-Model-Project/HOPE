...
if !@isdefined(validate_case_directory)
    ...
    function validate_model_mode(mode::AbstractString)
        if !(mode in VALID_MODEL_MODES)
            throw(
                ArgumentError(
                    "Invalid model mode: $mode. Must be one of: $(join(VALID_MODEL_MODES, ", "))",
                ),
            )
        end
        return true
    ...
    """
    function aggregate_capacity_data(
        df::DataFrame,
        group_cols::Vector{Symbol},
        sum_cols::Vector{Symbol},
    )
    ...
    function get_case_paths(case_name::String)
        ...
        modelcases_root =
            get(ENV, "HOPE_MODELCASES_PATH", joinpath(project_root, "ModelCases"))
        ...
        for attempt = 1:max_retries
            try
                ...
                if isa(e, SystemError) && attempt < max_retries
                    println(
                        "Warning: Failed to remove directory '$path' (attempt $attempt/$max_retries): $(e.msg)",
                    )
                ...
                elseif attempt == max_retries
                    println(
                        "Error: Could not remove directory '$path' after $max_retries attempts: $e",
                    )
                    return false
                end
            end
    ...
    function resource_aggregation_enabled(config_set::Dict)
        ...
        if haskey(config_set, "resource_aggregation")
            return parse_binary(
                config_set["resource_aggregation"],
    ...
    """
    function normalize_timeseries_time_columns!(
        df::DataFrame;
        context::AbstractString = "timeseries",
    )
        ...
        to_int(x, default::Int) =
            (ismissing(x) || string(x) == "") ? default : parse(Int, string(x))
        df[!, "Time Period"] = [to_int(df[r, "Time Period"], 1) for r = 1:n]
        df[!, "Hours"] = [to_int(df[r, "Hours"], r) for r = 1:n]
        return df
    end
    ...
    """
    function validate_aligned_time_columns!(
        ...
        if nrow(reference_df) != nrow(other_df)
            throw(
                ArgumentError(
                    "Timeseries row mismatch: reference has $(nrow(reference_df)) rows but $(other_name) has $(nrow(other_df)) rows.",
                ),
            )
        end
        if any(reference_df[!, "Time Period"] .!= other_df[!, "Time Period"]) ||
           any(reference_df[!, "Hours"] .!= other_df[!, "Hours"])
            throw(
                ArgumentError(
                    "Timeseries time mapping mismatch between reference and $(other_name). Columns 'Time Period' and 'Hours' must align row-by-row.",
                ),
            )
        end
        return true
    ...
    function build_time_period_hours(df::DataFrame)
        ...
        if t_vals != collect(1:length(t_vals))
            throw(
                ArgumentError(
                    "Time Period values must be contiguous 1..T. Found $(t_vals).",
                ),
            )
        end
    ...
    """
    function resolve_rep_day_mode(
        config_set::AbstractDict;
        context::AbstractString = "model",
    )
        ...
        if has_new_endogenous
            endogenous_rep_day =
                parse_binary_setting(config_set["endogenous_rep_day"], "endogenous_rep_day")
            if has_old_rep
                old_rep = parse_binary_setting(
                    config_set["representative_day!"],
                    "representative_day!",
                )
                if old_rep != endogenous_rep_day
                    println(
                        "Warning ($(context)): representative_day! conflicts with endogenous_rep_day. Using endogenous_rep_day=$(endogenous_rep_day).",
                    )
                end
            end
        elseif has_old_rep
            endogenous_rep_day = parse_binary_setting(
                config_set["representative_day!"],
                "representative_day!",
            )
            println(
                "Warning ($(context)): representative_day! is deprecated; use endogenous_rep_day.",
            )
        end

        if has_new_external
            external_rep_day =
                parse_binary_setting(config_set["external_rep_day"], "external_rep_day")
            if has_old_external
                old_external = parse_binary_setting(
                    config_set["external_rep_weights"],
                    "external_rep_weights",
                )
                if old_external != external_rep_day
                    println(
                        "Warning ($(context)): external_rep_weights conflicts with external_rep_day. Using external_rep_day=$(external_rep_day).",
                    )
                end
            end
        elseif has_old_external
            external_rep_day = parse_binary_setting(
                config_set["external_rep_weights"],
                "external_rep_weights",
            )
            println(
                "Warning ($(context)): external_rep_weights is deprecated; use external_rep_day.",
            )
        end
        ...
        if endogenous_rep_day == 1 && external_rep_day == 1
            throw(
                ArgumentError(
                    "Invalid representative-day settings: endogenous_rep_day=1 and external_rep_day=1 are mutually exclusive.",
                ),
            )
        end
