...
function apply_power_balance!(model::JuMP.Model, config::Dict, input_data::Dict)::Vector{JuMP.ConstraintRef}
    ...
    if mode == "GTEP"
        required_keys = ["T", "H_T", "I", "G", "S", "L"]
        if !check_required_data(input_data, required_keys)
            # Return placeholder constraint
            con_ref = @constraint(model, 0 == 0, base_name = "placeholder_power_balance")
            push!(refs, con_ref)
            return refs
        end
    ...
    if mode == "GTEP"
        ...
        required_keys = ["H", "I", "G", "S", "L"]
        if !check_required_data(input_data, required_keys)
            # Return placeholder constraint
            con_ref = @constraint(model, 0 == 0, base_name = "placeholder_power_balance_pcm")
            push!(refs, con_ref)
            return refs
        end
        ...
        for i in I, h in H
            # Generation in zone i - use safe column access
            gen_expr = @expression(model,
                sum(model[:p][g,h] for g in G if safe_get_column(input_data["Gendata"], g, [:Zone, :zone], 1) == i))
            
            # Storage discharge - charge in zone i - use safe column access
            storage_expr = @expression(model,
                sum(model[:dc][s,h] - model[:c][s,h] for s in S if safe_get_column(input_data["Storagedata"], s, [:Zone, :zone], 1) == i))
            
            # Net transmission flow into zone i - use safe column access
            trans_expr = @expression(model,
                sum(model[:f][l,h] for l in L if safe_get_column(input_data["Linedata"], l, [:Zone_to, :zone_to, :to_zone], 1) == i) -
                sum(model[:f][l,h] for l in L if safe_get_column(input_data["Linedata"], l, [:Zone_from, :zone_from, :from_zone], 1) == i))
...
function apply_storage_capacity_limit!(model::JuMP.Model, config::Dict, input_data::Dict)::Vector{JuMP.ConstraintRef}
    ...
    if mode == "GTEP"
        required_keys = ["T", "H_T", "S", "S_exist", "S_new"]
        if !check_required_data(input_data, required_keys)
            # Return placeholder constraint
            con_ref = @constraint(model, 0 == 0, base_name = "placeholder_storage_capacity")
            push!(refs, con_ref)
            return refs
        end
        ...
        for s in S_exist, t in T, h in H_T[t]
            scap = safe_get_column(input_data["Storagedata"], s, [:Max_Power_MW, Symbol("Max Power (MW)"), :max_power], 100.0)
            sc_rate = safe_get_column(input_data["Storagedata"], s, [:Charging_Rate, Symbol("Charging Rate"), :charging_rate], 1.0)
            sd_rate = safe_get_column(input_data["Storagedata"], s, [:Discharging_Rate, Symbol("Discharging Rate"), :discharging_rate], 1.0)
            ...
            secap = safe_get_column(input_data["Storagedata"], s, [:Capacity_MWh, Symbol("Capacity (MWh)"), :capacity], 400.0)
        ...
        for s in S_new, t in T, h in H_T[t]
            scap = safe_get_column(input_data["Storagedata_candidate"], s, [:Max_Power_MW, Symbol("Max Power (MW)"), :max_power], 100.0)
            sc_rate = safe_get_column(input_data["Storagedata_candidate"], s, [:Charging_Rate, Symbol("Charging Rate"), :charging_rate], 1.0)
            sd_rate = safe_get_column(input_data["Storagedata_candidate"], s, [:Discharging_Rate, Symbol("Discharging Rate"), :discharging_rate], 1.0)
            ...
            # Energy capacity limit
            secap = safe_get_column(input_data["Storagedata_candidate"], s, [:Capacity_MWh, Symbol("Capacity (MWh)"), :capacity], 400.0)
            con_ref3 = @constraint(model,
        ...
        if !check_required_data(input_data, required_keys)
            # Return placeholder constraint
            con_ref = @constraint(model, 0 == 0, base_name = "placeholder_storage_capacity_pcm")
            push!(refs, con_ref)
            return refs
        end
