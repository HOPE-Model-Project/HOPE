...
| $G^{UC}$ | Set of unit-commitment generators (Flag_UC = 1), index $g$, subset of $G$ |
...
| $NI_{i,h}$ | MW | Net interchange (imports minus exports) in zone $i$ in hour $h$; optional and exogenous — defaults to 0 when not modeled; in nodal modes $NodeNI_{n,h}$ provides bus-level values |
...
| $inj_{n,h}$ | MW | Net nodal power injection at bus $n$ in hour $h$ (PTDF mode, `network_model = 3`) |
| $o_{g,h}$ | units | Number of online units of UC generator $g$ in hour $h$ (integer or continuous relaxation) |
| $p_{g,h}$ | MW | Active power generation of unit $g$ in hour $h$ |
| $pmin_{g,h}$ | MW | Minimum online generation of UC unit $g$ in hour $h$; equals $(1-FOR_g)\,P^{min\text{-}unit}_g\,o_{g,h}$ |
| $pw_{g,w}$ | MWh | Total renewable generation of unit $g$ in state $w$ |
| $p_{i,h}^{LS}$ | MW | Zonal load shedding in zone $i$ in hour $h$; in nodal modes this is allocated to buses via bus load-share weights |
...
| $sd_{g,h}$ | units | Shutdown count of UC generator $g$ at the beginning of hour $h$ |
| $soc_{s,h}$ | MWh | State of charge level of storage $s$ in hour $h$ |
| $su_{g,h}$ | units | Startup count of UC generator $g$ at the beginning of hour $h$ |
| $\theta_{n,h}$ | rad | Voltage angle at bus $n$ in hour $h$ (angle-based DCOPF, `network_model = 2`) |
