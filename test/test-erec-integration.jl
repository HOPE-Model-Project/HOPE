...
@testset "EREC Integration" begin
    if get(ENV, "HOPE_RUN_EREC_INTEGRATION", "0") != "1"
        ...
        missing_cases = [
            case_name for case_name in required_cases
            if !isdir(joinpath(EREC_PROJECT_ROOT, "ModelCases", case_name))
        ]
        ...
        @testset "Candidate Build Reconstruction" begin
            mktempdir() do tmpdir
                src_case = joinpath(EREC_PROJECT_ROOT, "ModelCases", "MD_GTEP_erec_test_case")
                ...
                candidate_gen_rows = fixed_gens[string.(fixed_gens[:, "EREC_Source"]) .== "candidate", :]
                candidate_storage_rows = fixed_storage[string.(fixed_storage[:, "EREC_Source"]) .== "candidate", :]
                @test all(HOPE.to_float_erec.(candidate_gen_rows[:, "Pmax (MW)"]) .> 0.0)
                @test all(HOPE.to_float_erec.(candidate_storage_rows[:, "Max Power (MW)"]) .> 0.0)

                afdata = res["fixed_fleet_input"]["AFdata"]
                @test size(afdata, 2) == nrow(fixed_gens) + 4
                @test all(("G$(i)" in names(afdata)) for i in 1:nrow(fixed_gens))
            end
        end

        @testset "Candidate Scope All" begin
            mktempdir() do tmpdir
                src_case = joinpath(EREC_PROJECT_ROOT, "ModelCases", "MD_GTEP_erec_candidate_case")
                ...
                @test any((results.Source .== "candidate") .& (results.BaselinePowerMW .> 0.0))
                @test any((results.Source .== "candidate") .& (results.BaselinePowerMW .== 0.0))
                @test all((isnan(x) || x <= 1.0 + 1.0e-8) for x in results.EREC)

                @test haskey(res["output_paths"], "gendata_with_erec_cc")
                cc_gendata = CSV.read(res["output_paths"]["gendata_with_erec_cc"], DataFrame)
        ...
        @testset "Aggregated Full Chronology Regression" begin
            mktempdir() do tmpdir
                src_case = joinpath(EREC_PROJECT_ROOT, "ModelCases", "MD_GTEP_erec_fullchron_stressed_case")
                tmp_case = joinpath(tmpdir, "MD_GTEP_erec_fullchron_stressed_case")
                cp(src_case, tmp_case; force=true)

                config = open(joinpath(tmp_case, "Settings", "HOPE_model_settings.yml")) do io
                    YAML.load(io)
                end
                ...
                wind_rows = findall((string.(gendata[:, "Zone"]) .== "APS_MD") .& (string.(gendata[:, "Type"]) .== "WindOn"))
                solar_rows = findall((string.(gendata[:, "Zone"]) .== "APS_MD") .& (string.(gendata[:, "Type"]) .== "SolarPV"))
                thermal_rows = findall((string.(gendata[:, "Zone"]) .== "PEPCO") .& (string.(gendata[:, "Type"]) .== "NGCC_CCS"))
                ...
                @test minimum(Float64.(afdata[:, wind_col])) < maximum(Float64.(afdata[:, wind_col]))
                @test minimum(Float64.(afdata[:, solar_col])) < maximum(Float64.(afdata[:, solar_col]))
                ...
                wind_erec = results[(results.Technology .== "WindOn") .& (results.Zone .== "APS_MD"), "EREC"][1]
                solar_erec = results[(results.Technology .== "SolarPV") .& (results.Zone .== "APS_MD"), "EREC"][1]
                thermal_erec = results[(results.Technology .== "NGCC_CCS") .& (results.Zone .== "PEPCO"), "EREC"][1]
                hydro_erec = results[(results.Technology .== "Hydro") .& (results.Zone .== "APS_MD"), "EREC"][1]
        ...
        @testset "Full Chronology Storage Regression" begin
            mktempdir() do tmpdir
                ...
                @test all((isnan(x) || (x >= -1.0e-8 && x <= 1.0 + 1.0e-8)) for x in results.EREC)
                ...
                @test battery_row["BaselineEnergyMWh"] / battery_row["BaselinePowerMW"] ≈ 4.0
                ...
                @test HOPE.to_float_erec(fixed_storage[1, "Capacity (MWh)"]) / HOPE.to_float_erec(fixed_storage[1, "Max Power (MW)"]) ≈ 4.0
        ...
        @testset "Full Chronology Storage All Scope" begin
            mktempdir() do tmpdir
                ...
                @test Set(candidate_rows.Zone) == Set(["APS_MD", "BGE", "DPL_MD", "PEPCO"])
                ...
                cc_storagedata = CSV.read(res["output_paths"]["storagedata_with_erec_cc"], DataFrame)
        ...
        @testset "Transmission Build Reconstruction" begin
            mktempdir() do tmpdir
                src_case = joinpath(EREC_PROJECT_ROOT, "ModelCases", "MD_GTEP_erec_linebuild_case")
                ...
                @test all((isnan(x) || (x >= -1.0e-8 && x <= 1.0 + 1.0e-8)) for x in res["erec_results"].EREC)

                line_builds = [value(res["baseline_model"][:y][l]) for l in axes(res["baseline_model"][:y], 1)]
        ...
        mktempdir() do tmpdir
            src_case = joinpath(EREC_PROJECT_ROOT, "ModelCases", "MD_GTEP_erec_stressed_case")
            ...
            weights = Dict(Int(row["Time Period"]) => Float64(row["Weight"]) for row in eachrow(weights_df))
            I = axes(model[:p_LS], 1)
            H = axes(model[:p_LS], 2)
            ...
            @test eue_hours_weighted < sum(Float64(row["Weight"]) * 24.0 for row in eachrow(weights_df))
