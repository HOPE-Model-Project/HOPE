...
function build_three_day_timeseries()
    ...
    loaddata = DataFrame(
        Symbol("Time Period") => time_period,
        :Month => month,
        :Day => day,
        :Hours => hour,
        :Z1 => load,
        :NI => zeros(length(load)),
    )
    afdata = DataFrame(
        Symbol("Time Period") => time_period,
        :Month => month,
...
""",
        ...
        resolved =
            HOPE.resolve_rep_day_time_periods(Dict("rep_day_settings" => rep_settings))
    ...
    rep_multi = HOPE.build_endogenous_rep_periods(
        multi_load,
        multi_af,
        ["Z1"],
        ["G1"],
    ...
    kmeans_multi_config = Dict{String,Any}(
        "rep_day_settings" => Dict(
            "time_periods" => Dict(1 => [1, 1, 1, 4]),
    ...
    rep_kmeans = HOPE.build_endogenous_rep_periods(
        multi_load,
        multi_af,
        ["Z1"],
        ["G1"],
    ...
    @test rep_kmeans["T"] == [1, 2]
    @test sort(collect(values(rep_kmeans["N"]))) == [2.0, 2.0]
    @test all(rep_kmeans["metadata"][!, "SelectionType"] .== "cluster_centroid")
    @test all(rep_kmeans["metadata"][!, "Method"] .== "joint_daily_kmeans")
    centroid_profiles = [rep_kmeans["Load_rep"][t][!, "Z1"] for t in rep_kmeans["T"]]
    @test any(all(isapprox.(profile, fill(0.0, 24))) for profile in centroid_profiles)
    @test any(all(isapprox.(profile, fill(10.0, 24))) for profile in centroid_profiles)
    ...
    rep_linked = HOPE.build_endogenous_rep_periods(
        multi_load,
        multi_af,
        ["Z1"],
        ["G1"],
    ...
    kmeans_linked_config = Dict{String,Any}(
        "rep_day_settings" => Dict(
            "time_periods" => Dict(1 => [1, 1, 1, 4]),
            "clustering_method" => "kmeans",
            "feature_mode" => "joint_daily",
    ...
    rep_kmeans_linked = HOPE.build_endogenous_rep_periods(
        multi_load,
        multi_af,
        ["Z1"],
        ["G1"],
        kmeans_linked_config,
    )
    kmeans_linkage = rep_kmeans_linked["storage_linkage"]
    @test kmeans_linkage !== nothing
    @test nrow(kmeans_linkage["day_assignments"]) == 4
    @test sort(kmeans_linkage["day_assignments"][!, "RepresentativePeriod"]) == [1, 1, 2, 2]
    ...
    rep_planning = HOPE.build_endogenous_rep_periods(
        planning_load,
        planning_af,
        ["Z1"],
        ["G1", "G2"],
        planning_config;
        generator_data = planning_generator_df,
    )
    @test rep_planning["metadata"][1, "SelectedDay"] == 2
    @test rep_planning["metadata"][1, "Method"] == "planning_features_kmedoids"

    planning_kmeans_config = Dict{String,Any}(
        "rep_day_settings" => Dict(
            "time_periods" => Dict(1 => [1, 1, 1, 3]),
            "clustering_method" => "kmeans",
            "feature_mode" => "planning_features",
    ...
    rep_planning_kmeans = HOPE.build_endogenous_rep_periods(
        planning_load,
        planning_af,
        ["Z1"],
        ["G1", "G2"],
        planning_kmeans_config;
        generator_data = planning_generator_df,
    )
    @test rep_planning_kmeans["metadata"][1, "Method"] == "planning_features_kmeans"
    @test rep_planning_kmeans["metadata"][1, "SelectionType"] == "cluster_centroid"
    @test all(isapprox.(rep_planning_kmeans["Load_rep"][1][!, "Z1"], fill(5.0, 24)))

    extreme_load, extreme_af =
        build_four_day_extreme_timeseries([0.0, 1.0, 2.0, 10.0], [1.0, 1.0, 1.0, 1.0])
    ...
    rep_extreme = HOPE.build_endogenous_rep_periods(
        extreme_load,
        extreme_af,
        ["Z1"],
        ["G1"],
        extreme_config;
        generator_data = generator_df,
    )
    ...
    wind_load, wind_af =
        build_four_day_extreme_timeseries([5.0, 5.0, 5.0, 5.0], [0.9, 0.8, 0.7, 0.1])
    ...
    rep_wind_extreme = HOPE.build_endogenous_rep_periods(
        wind_load,
        wind_af,
        ["Z1"],
        ["G1"],
        wind_config;
        generator_data = wind_generator_df,
    )
    ...
    wrap_config = Dict{String,Any}(
        "rep_day_settings" => Dict("time_periods" => Dict(1 => [12, 31, 1, 1])),
    )
    wrap_af = DataFrame(
        Symbol("Time Period") => vcat(fill(1, 24), fill(1, 24)),
        :Month => vcat(fill(12, 24), fill(1, 24)),
    ...
    weights =
        HOPE.endogenous_rep_day_weights(wrap_loaddata, wrap_af, ["Z1"], ["G1"], wrap_config)
    @test weights[1] == 2.0

    refine_load, refine_af =
        build_four_day_extreme_timeseries([0.0, 1.0, 2.0, 10.0], [1.0, 1.0, 1.0, 1.0])
    refine_load = vcat(
        refine_load,
        DataFrame(
    ...
    rep_refine = HOPE.build_endogenous_rep_periods(
        refine_load,
        refine_af,
        ["Z1"],
        ["G1"],
        refine_config;
        generator_data = refine_generator_df,
    )
    ...
    refine_kmeans_config = Dict{String,Any}(
        "rep_day_settings" => Dict(
            "time_periods" => Dict(1 => [1, 1, 1, 5]),
            "clustering_method" => "kmeans",
            "feature_mode" => "joint_daily",
    ...
    rep_refine_kmeans = HOPE.build_endogenous_rep_periods(
        refine_load,
        refine_af,
        ["Z1"],
        ["G1"],
        refine_kmeans_config;
        generator_data = refine_generator_df,
    )
    @test sum(values(rep_refine_kmeans["N"])) == 5.0
    @test "cluster_centroid" in rep_refine_kmeans["metadata"][!, "SelectionType"]
    @test "extreme_day" in rep_refine_kmeans["metadata"][!, "SelectionType"]
    @test "refinement_day" in rep_refine_kmeans["metadata"][!, "SelectionType"]
    @test rep_refine_kmeans["storage_linkage"] !== nothing

    overlap_config = Dict{String,Any}(
        "rep_day_settings" =>
            Dict("time_periods" => Dict(1 => [1, 1, 1, 2], 2 => [1, 2, 1, 3])),
    )
    @test_throws ArgumentError HOPE.build_endogenous_rep_periods(
        loaddata,
        afdata,
        ["Z1"],
        ["G1"],
    ...
    gap_config = Dict{String,Any}(
        "rep_day_settings" =>
            Dict("time_periods" => Dict(1 => [1, 1, 1, 1], 2 => [1, 3, 1, 3])),
    )
    @test_throws ArgumentError HOPE.build_endogenous_rep_periods(
        loaddata,
        afdata,
    ...
    @test_throws ArgumentError HOPE.validate_external_rep_day_inputs(
        external_load,
        external_af,
        external_weight_df,
    )
    ...
    @test_throws ArgumentError HOPE.validate_external_rep_day_inputs(
        external_load,
        external_af,
        external_bad_weight_df,
    )

    external_ok_weight_df = DataFrame(Symbol("Time Period") => [1], :Weight => [3.0])
    external_n = HOPE.validate_external_rep_day_inputs(
        external_load,
        external_af,
        external_ok_weight_df,
    )
    @test external_n[1] == 3.0

    mktempdir() do outdir
        HOPE.write_rep_day_audit_outputs(
            outdir,
            1,
            0,
            rep_refine["N"];
            rep_period_data = rep_refine,
        )
        ...
        weight_check =
            CSV.read(joinpath(outdir, "representative_period_weight_check.csv"), DataFrame)
