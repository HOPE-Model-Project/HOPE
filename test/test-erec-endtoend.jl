...
function build_tiny_erec_case_tables()
    ...
    zonedata =
        DataFrame(:Zone_id => ["Z1"], Symbol("Demand (MW)") => [120.0], :State => ["S1"])
    ...
    rpspolicies = DataFrame(:From_state => ["S1"], :To_state => ["S1"], :RPS => [0.0])
    ...
    repweights = DataFrame(Symbol("Time Period") => [1], :Weight => [365.0])
...
function assert_same_erec_results(df_a::DataFrame, df_b::DataFrame; atol::Float64 = 1.0e-6)
    ...
    for row_idx = 1:nrow(a), col in names(a)
        aval = a[row_idx, col]
        bval = b[row_idx, col]
...
@testset "EREC End-to-End Saved Baseline Workflows" begin
    mktempdir() do tmpdir
        ...
        @test isfile(
            joinpath(run_res["output_path"], "postprocess_snapshot", "metadata.yml"),
        )
        ...
        @test isapprox(
            erec_from_results["baseline_eue"],
            erec_from_output["baseline_eue"];
            atol = 1.0e-6,
            rtol = 0.0,
        )
        @test nrow(erec_from_results["erec_results"]) == 2
        @test nrow(erec_from_output["erec_results"]) == 2
        @test Set(erec_from_results["erec_results"].ResourceType) ==
              Set(["generator", "storage"])
        @test Set(erec_from_output["erec_results"].ResourceType) ==
              Set(["generator", "storage"])
        @test all(
            (isnan(x) || (x >= -1.0e-8 && x <= 1.0 + 1.0e-8)) for
            x in erec_from_results["erec_results"].EREC
        )
        @test all(
            (isnan(x) || (x >= -1.0e-8 && x <= 1.0 + 1.0e-8)) for
            x in erec_from_output["erec_results"].EREC
        )
        @test erec_from_results["baseline_model"] === run_res["solved_model"]
        @test erec_from_output["baseline_model"] === nothing

        assert_same_erec_results(
            erec_from_results["erec_results"],
            erec_from_output["erec_results"],
        )

        @test isfile(erec_from_results["output_paths"]["erec_results"])
        @test isfile(erec_from_output["output_paths"]["erec_results"])

        snapshot_settings = joinpath(
            run_res["output_path"],
        ...
        @test isapprox(
            erec_preserve_voll["baseline_eue"],
            erec_from_output["baseline_eue"];
            atol = 1.0e-6,
            rtol = 0.0,
        )
