...
@testset "Full chronological time mapping" begin
    full_year_sample = DataFrame(Symbol("Time Period") => fill(1, 3), :Hours => [1, 2, 3])
    ...
    singleton_periods = DataFrame(Symbol("Time Period") => [1, 2, 3], :Hours => [1, 2, 3])
    bad_periods, bad_hours_by_period, _, _ = HOPE.build_time_period_hours(singleton_periods)
